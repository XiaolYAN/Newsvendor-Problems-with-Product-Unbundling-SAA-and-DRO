    ...
    if DisType_Demand == "Normal":
        ...
        for i in range(TrainSampleSize):
            ...
            if distribution == 0:
                ...
                temp_sample = np.clip(temp_sample, 0, None)
                ...
                TrainSamples[0][i] = int(exponential_samples_1)
                TrainSamples[1][i] = int(exponential_samples_2)
            else:
                correlation_matrix = np.array([[1, rho], [rho, 1]])
                L = np.linalg.cholesky(correlation_matrix)
                standard_uniform_samples = np.random.uniform(size=(1, 2))
        ...
        for i in range(TestSampleSize):
            ...
            if distribution == 0:
                ...
                TestSamples[0][i] = int(exponential_samples_1)
                TestSamples[1][i] = int(exponential_samples_2)
            else:
                correlation_matrix = np.array([[1, rho], [rho, 1]])
                L = np.linalg.cholesky(correlation_matrix)
                standard_uniform_samples = np.random.uniform(size=(1, 2))
        ...
        TrainSamples = np.round(TrainSamples)
        TrainSamples = np.clip(TrainSamples, 0, None)
        TrainSamples = np.array(TrainSamples).reshape(-1, 2).transpose()
        TestSamples = []
        ...
        TestSamples = np.round(TestSamples)
        TestSamples = np.clip(TestSamples, 0, None)
        TestSamples = np.array(TestSamples).reshape(-1, 2).transpose()
    else:
        pass
